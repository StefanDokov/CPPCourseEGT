...
Game::Game()
{
    ...
    Game::currentRow = 1;
    isIt = false;
    Game::colorizm = { 255, 255, colorizm.x / 2, colorizm.y / 2 };
}
...
bool Game::init(const char* title, int xpos, int ypos, int width, int height, int flags)
{
    ...
    if (window != 0) {
        ...
        if (renderer != 0) {
            ...
            TextureManager::Instance()->loadTexture("assets/round.jpg","round", renderer);
...
void Game::render()
{
    SDL_SetRenderDrawColor(renderer, colorizm.x, colorizm.w, colorizm.y, 0xFF);
    ...
    SDL_SetRenderDrawColor(renderer, colorizm.y, colorizm.x, colorizm.h, 0xFF);
    ...
    //TextureManager::Instance()->drawOneFrameFromTexture("round", ww / 2, wh / 2, 83, 84, currentRow, currentFrame, renderer);
...
void Game::update()
{
    ...
    //std::cout << "SDL_TICKS / 100 % 12: " << int(((SDL_GetTicks() / 100) % 12)) << "\n\n";
    //std::cout << "Roll          :" << currentRow << "\n";

    //    currentFrame = int(((SDL_GetTicks() / 100) % 12)); // % 10 for 10 frames
    //    if (currentFrame == 11) {
    //        isIt = true;
    //        
    //    }
...
void Game::handleEvents()
{
    ...
    if (SDL_PollEvent(&event)) {
        ...
        case SDL_MOUSEMOTION: {
            colorizm.x = event.motion.x / 4;
            colorizm.y = event.motion.y / 3;
